...
The original row 5/1u printed geometry and its runtime assets remain unchanged. A separate KeyV2 development recipe generated isolated corner-radius probes (0.5 and 1.5 mm around 1 mm) and depth-offset probes (−0.5 and +0.5 mm around zero). OpenSCAD reports manifold blanks; initial bounds and volume measurements are in [the customization plan](docs/plans/oem-customization.md). These are exploratory source parameters, not physically validated ranges. Radius slightly changes maximum height; depth offset slightly changes the outer footprint. The first radius probes had matching exterior meshes and locally generated two-part 3MF tests. Digital checks found no difference in the bounded central insertion region or Body/Legend overlap, but do not validate complete socket fit, physical clearance, or print quality. No physical fit or printability claim follows from these probes.

The user subsequently chose to offer bounded adjustments without printing each combination first. The editor therefore generates KeyV2 blank/exterior pairs at radius 0.50–1.50 mm and row-relative depth offset −0.50–+0.50 mm in 0.25 mm steps for the existing catalogue. This is a software range, not an OEM-certified tolerance or evidence that all combinations fit the K2. The untouched radius 1 mm / depth offset 0 preset retains its physically checked mesh. Nondefault heights are labeled OEM derived. Keep new combinations experimental until individual physical feedback is available; do not silently revise the known-good default.
